...
function resolveMicrosoftCallbackUrl(req) {
  const envRedirect = process.env.MICROSOFT_AUTH_REDIRECT_URI?.trim() || process.env.MICROSOFT_REDIRECT_URI?.trim()
  const frontendUrl = process.env.FRONTEND_URL?.trim()?.replace(/\/$/, '')
  if (process.env.NODE_ENV === 'production' && envRedirect) return envRedirect
