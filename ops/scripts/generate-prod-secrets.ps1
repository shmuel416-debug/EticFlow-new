...
if ($KeyVaultName) {
  ...
  Write-Host "Key Vault upload complete. Secret values were not printed."
  Write-Host "Secrets stored: jwt-secret-current, db-password, calendar-token-encryption-key"
} else {
